...
public class Converter {
        ...
        private static final String[][] SYMTAB = new String[9][2];

    public static String[] initializeOPTAB (String str) {
        ...
        for(int i=0;i<OPTAB.length;i++){
            ...
            if(str.toUpperCase().trim().equals(OPTAB[i][0])){
                for(int j =0;j<OPTAB[i].length;j++){
                      returnval[j] = OPTAB[i][j];
                }
            }
        }
        return returnval;
    ...
    public static String[] initializeSYMTAB (String str) {
        
        SYMTAB[0] = new String[] {"A", "0"};
        SYMTAB[1] = new String[] {"X", "1"};
        SYMTAB[2] = new String[] {"L", "2"};
        SYMTAB[3] = new String[] {"B", "3"};
        SYMTAB[4] = new String[] {"S", "4"};
        SYMTAB[5] = new String[] {"T", "5"}; 
        SYMTAB[6] = new String[] {"F", "6"}; 
        SYMTAB[7] = new String[] {"PC", "8"};
        SYMTAB[8] = new String[] {"SW", "9"};
        
        String[] returnval = new String[2];
        for(int i=0;i<SYMTAB.length;i++){
            
            if(str.toUpperCase().trim().contains(SYMTAB[i][0])){
                for(int j =0;j<SYMTAB[i].length;j++){
                      returnval[j] = SYMTAB[i][j];
                }
            }
        }
        return returnval;
    ...
    String Format(String inst){
        String[] temp = Converter.initializeOPTAB(inst);
        return temp[1];
    }
    ...
    String register_Translation(String inst){
        String[] temp = Converter.initializeSYMTAB(inst);
       if(temp[1]!=null){ 
        return temp[1];
        }else return "0";
    }
    
  String LengthCheck(String str , int len){
         if(str.length()<len){
         String temp1="";
